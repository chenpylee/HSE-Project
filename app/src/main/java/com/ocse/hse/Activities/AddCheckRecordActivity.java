...
public class AddCheckRecordActivity extends Activity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        currentTaskID=ApplicationController.getCurrentTaskID();
        organSpinner=(Spinner)findViewById(R.id.organSpinner);
        organList=JCDWInfo.getJCDWByJR(currentTaskID);
        if(organList.size()>0)
        {
            selectedOrganID=Integer.toString(organList.get(0).getJR_DEPTID());
        }
        List<String> list = new ArrayList<String>();
        for(int i=0;i<organList.size();i++)
        {
            list.add(organList.get(i).getJR_DWMC());
        }
        ArrayAdapter<String> spinnerAdapter = new ArrayAdapter<String>(this,
                android.R.layout.simple_spinner_item, list);
        spinnerAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
        organSpinner.setAdapter(spinnerAdapter);
        organSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            @Override
            public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
               selectedOrganID=Integer.toString(organList.get(position).getJR_DEPTID());
            }
        ...
        if(ruleItem!=null)
        {
            txtRuleLv3.setText(ruleItem.getRuleLv3());
            txtRuleContent.setText(ruleItem.getRuleContent());
        }
    ...
    private void quitActivity()
    {
        ...
        if(!ok)
        {
            ...
            dialog.setTitle("隐患记录");
            dialog.setMessage("保存并退出隐患记录？");
            dialog.show();
        }
        else{
            finish();
    ...
    private void saveRecord(){
        ...
        RecordInfo recordInfo=new RecordInfo(selectedOrganID,ruleItem,record_description,imageList,record_contact, record_phone,record_created, record_updated);
